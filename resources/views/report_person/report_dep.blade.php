...
@section('title', 'PK-OFFICER || บัญชีรายงานวันลา')
@section('content')
    <script>
        function TypeAdmin() {
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-xl-12">
                <form action="{{ url('report_dep') }}" method="POST">
                    @csrf
                    <div class="row">
                        <div class="col"></div>
                        ...
                        <div class="col-md-2 text-center">
                            <div class="input-group">
                                <select type="text" class="form-control" name="HR_DEPARTMENT_ID" id="HR_DEPARTMENT_ID">
                                    <option value="">--เลือก--</option>
                                    @foreach ($dep as $itemdep)
                                    @if ($depid == $itemdep->HR_DEPARTMENT_ID)
                                    <option value="{{$itemdep->HR_DEPARTMENT_ID}}" selected>{{$itemdep->HR_DEPARTMENT_NAME}}</option>
                                    @else
                                    <option value="{{$itemdep->HR_DEPARTMENT_ID}}">{{$itemdep->HR_DEPARTMENT_NAME}}</option>
                                    @endif
