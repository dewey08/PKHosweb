...
@section('content')
    ...
    <div id="preloader">
        ...
        <form action="{{ route('acc.acc_stm') }}" method="POST">
            @csrf
            <div class="row"> 
                <div class="col"></div>
                                ...
                                {{-- <table id="example" class="table table-hover table-sm table-light dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;"> --}}
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-center" width="8%">00000</th>  
                                            <th class="text-center">stm</th>
                                            <th class="text-center">ชดเชย</th> 
                                            <th class="text-center" width="7%">pang_stamp_send</th>
                                            {{-- <th class="text-center">pang_stamp_id</th> --}}
                                            <th class="text-center">pang_stamp</th>
                                            <th class="text-center" width="10%">stm</th> 
                                            <th class="text-center">pang_stamp_stm_rep</th>
                                            <th class="text-center">pang_stamp_edit_send_id</th>
                                            <th class="text-center">receipt_n</th>
                                            <th class="text-center">receipt_date</th> 
                                            <th class="text-center" width="15%">pt_name</th> 
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php $i = 1; ?>
                                        @foreach ($acc_stm as $item) 
                                            <tr >                                                  
                                                ...
                                                <td class="text-center" width="5%">{{ $item->pang_stamp_vstdate }}</td>  
                                                {{-- <td class="p-2" width="10%">{{ $item->pang_stamp_nhso }}</td>  --}}
                                                <td class="text-center" width="5%">{{ $item->pang_stamp_uc_money }}</td>  
                                                <td class="text-center" style="color:rgb(73, 147, 231)" width="5%">{{ $item->pang_stamp_uc_money_kor_tok }}</td> 
                                                <td class="p-2" width="10%">{{ $item->stm }}</td>  
                                                <td class="text-center" style="color:rgb(216, 95, 14)" width="5%">{{ $item->pang_stamp_uc_money_minut_stm_money }}</td> 
                                                <td class="text-center" width="5%">{{ $item->pang_stamp_send }}</td>  
                                                {{-- <td class="p-2">{{ $item->pang_stamp_id }}</td>  --}}
                                                <td class="text-center">{{ $item->pang_stamp }}</td> 
                                                <td class="p-2">{{ $item->pang_stamp_stm_file_name }}</td>  
                                                <td class="text-center">{{ $item->pang_stamp_stm_rep }}</td> 
                                                <td class="text-center">{{ $item->pang_stamp_edit_send_id }}</td> 
                                                <td class="text-center">{{ $item->receipt_n }}</td> 
                                                <td class="text-center">{{ $item->receipt_date }}</td> 
                                                <td class="p-2" width="15%">{{ $item->pt_name }}</td>  
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
