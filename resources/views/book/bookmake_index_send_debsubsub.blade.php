...
@section('content')
    <script>
        ...
        function bookmake_senddepsubsub_destroy(senddepsubsub_id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ url('book/bookmake_senddepsubsub_destroy') }}" + '/' + senddepsubsub_id,
                        type: 'DELETE',
                        data: {
                                ...
                                <div class="row text-center">
                                    ...
                                    <div class="col-md-4 mt-5 mb-3">
                                        <div class="form-group">
                                            <select id="DEPARTMENT_SUB_SUB_ID" name="DEPARTMENT_SUB_SUB_ID" class="form-control"
                                                style="width: 100%" >
                                                <option value=""></option>
                                                @foreach ($department_sub_sub as $depart_ss)
                                                    <option value="{{ $depart_ss->DEPARTMENT_SUB_SUB_ID }}"> {{ $depart_ss->DEPARTMENT_SUB_SUB_NAME }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="col-md-12 ">
                                    <div class="table-responsive">
                                        <table class="table table-hover table-bordered table-sm myTable"
                                            ...
                                            <tbody>
                                                <?php $num = 1; ?>
                                                @foreach ($book_senddep_subsub as $item3)
                                                    <tr id="sid{{ $item3->senddepsubsub_id }}">
                                                        <td class="text-center" width="5%">{{ $num++ }}</td>
                                                        <td class="p-2">{{ $item3->senddep_depsubsub_name }}</td>
                                                        <td class="p-2">{{ $item3->objective_name }}</td>
                                                        <td class="text-center" width="10%">

                                                            <a class="text-danger" href="javascript:void(0)"
                                                                onclick="bookmake_senddepsubsub_destroy({{ $item3->senddepsubsub_id }})"
...
<script>
    $(document).ready(function() {
        ...
        $('#saveBtn').click(function() {
                // alert('okkkkk');
                var dep_subsub_id = $('#DEPARTMENT_SUB_SUB_ID').val(); // 
                ...
                $.ajax({
                    url: "{{ route('book.bookmake_index_senddepsubsub') }}",
                    type: "POST",
                    dataType: 'json',
                    data: {
                        dep_subsub_id,
