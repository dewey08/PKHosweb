    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-xl-12">
                    ...
                    <div class="row">
                        <div class="col">
                            <h5 class="mb-sm-0">รายงานข้อมูลสิทธิ พรบ.ผู้ป่วยใน </h5>
                        </div>
                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">วงเงินสูงสุด</th>
                            ...
                            <tbody>
                                <?php $i = 1; ?>
                                @foreach ($data_repipdsubvn as $item)
                                    <tr>
                                        <td>{{ $i++ }}</td>
                                        <td>{{$item->hn}}</td> 
                                        <td>{{$item->an}}</td> 
                                        ...
                                        <td>{{ $item->amount }}</td>
                                        <td> 
                                            <a href="{{url('prb_repipd_subsuban/'.$item->an.'/'.$startdate.'/'.$enddate)}}" target="_blank">{{ $item->income }}</a> 
                                        </td>
                                        <td>{{ $item->paid_money }}</td>
                                        <td>{{ $item->remain_money }} </td>
                                        <td>{{ $item->max_debt_amount }} </td>

                                        <td>{{ $item->nhso_docno }} </td>
                                        <td>{{ $item->nhso_ownright_pid }} </td>
                                        <td>{{ $item->nhso_govname }} </td>
                                        <td>{{ $item->note }} </td>
                                    </tr>
                                @endforeach
                            </tbody>
