...
?>
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-xl-12">
                <form action="{{ route('prb.prb_repipdover') }}" method="POST">
                    @csrf
                    <div class="row">
                        <div class="col">
                            <h5 class="mb-sm-0">รายงานการลงข้อมูลผู้ป่วยใน พรบ. ที่ Admit อยู่แต่วงเงินเกิน 30000</h5>
                        </div>
                ...
                <div class="card-body py-0 px-2 mt-2">
                    <div class="table-responsive"> 
                        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                            ...
                            <tbody>
                                <?php $i = 1; ?>
                                @foreach ($data_repipdover as $item)
                                    <tr>
                                        <td>{{ $i++ }}</td> 
                                        @if ($item->months == '1')
                                        ...
                                        @endif
                                        {{-- <td>
                                            <a href="{{url('prb_repipdover_subhn/'.$item->months.'/'.$startdate.'/'.$enddate)}}" target="_blank">{{ $item->hn }}</a>   
                                        </td>
                                        <td>
                                            <a href="{{url('prb_repipdover_suban/'.$item->months.'/'.$startdate.'/'.$enddate)}}" target="_blank">{{ $item->an }}</a>    
                                        </td>
                                        <td>
                                            <a href="{{url('prb_repipdover_subreq/'.$item->months.'/'.$startdate.'/'.$enddate)}}" target="_blank">{{ $item->nhso_ownright_pidover }}</a>  
                                         </td>
                                        <td>
                                            <a href="{{url('prb_repipdover_subnoreq/'.$item->months.'/'.$startdate.'/'.$enddate)}}" target="_blank">{{ $item->claim_code }}</a>  
                                        </td> --}}
                                        <td>
                                             {{ $item->hn }}   
                                        </td>
