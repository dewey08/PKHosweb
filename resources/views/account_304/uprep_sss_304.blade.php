...
<script>
    ...
    <div class="tabs-animation p-3 ">
        ...
        <form action="{{ url('uprep_sss_304') }}" method="GET">
            @csrf
            <div class="row"> 
                <div class="col"></div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.editModal', function() {
                var acc_1102050101_304_id = $(this).val(); 
                // alert(acc_1102050101_304_id);
                $('#editModal').modal('show');
                $.ajax({
                    type: "GET",
                    url: "{{ url('uprep_sss_304edit') }}" + '/' + acc_1102050101_304_id,
                    success: function(data) {
                        console.log(data.data_show.acc_1102050101_304_id); 
                        ...
                        $('#editid').val(data.data_show.acc_1102050101_304_id)
                    },
                });
            });
            ...
            $('#Updatedata').click(function() {
                    ...
                    var acc_1102050101_304_id = $('#editid').val();

                    $.ajax({
                        url: "{{ route('acc.uprep_sss_304_update') }}",
                        type: "POST",
                        dataType: 'json',
                        data: {
                            recieve_true,difference,recieve_no,recieve_date,user_id,acc_1102050101_304_id
                        },
                        success: function(data) {
                            if (data.status == 200) {
                                Swal.fire({
