...
use App\Models\Acc_1102050101_303;
...
class Account303Controller extends Controller
 {
    // ***************** 301********************************
     
    public function account_303_dash(Request $request)
    {
       ...
       if ($acc_trimart_id == '') {
        ...
        return view('account_303.account_303_dash',[
            'startdate'        => $startdate,
            'enddate'          => $enddate,
    ...
    public function account_303_pull(Request $request)
    {
        $datenow = date('Y-m-d');
        $months = date('m');
        $year = date('Y');
        ...
        if ($startdate == '') {
            ...
            $acc_debtor = DB::select('
                ...
                WHERE a.account_code="1102050101.303"
                AND a.stamp = "N"
                group by a.vn
        ...
        return view('account_303.account_303_pull',[
            'startdate'     =>     $startdate,
            'enddate'       =>     $enddate,
            'acc_debtor'    =>     $acc_debtor,
        ]);
    }
    public function account_303_pulldata(Request $request)
    {
        $datenow = date('Y-m-d');
        ...
        $type = DB::connection('mysql')->select('
            SELECT pttype from acc_setpang_type WHERE pttype IN (SELECT pttype FROM acc_setpang_type WHERE pang ="1102050101.303")
        ');
        ...
        $acc_debtor = DB::connection('mysql2')->select('
            SELECT v.vn,ifnull(o.an,"") as an,o.hn,pt.cid
                    ...
                    ,e.code as acc_code
                    ,"1102050101.303" as account_code
            ...
            AND vp.pttype IN (SELECT pttype FROM pkbackoffice.acc_setpang_type WHERE pang ="1102050101.303")             
        ...
        foreach ($acc_debtor as $key => $value) {
                    $check = Acc_debtor::where('vn', $value->vn)->where('account_code','1102050101.303')->count(); 
                    if ($check == 0) {
                        Acc_debtor::insert([
                            'hn'                 => $value->hn,
                            'an'                 => $value->an,
    ...
    public function account_303_checksit(Request $request)
    {
        $datestart = $request->datestart;
        $dateend = $request->dateend;
        $date = date('Y-m-d');
        
        $data_sitss = DB::connection('mysql')->select('SELECT vn,an,cid,vstdate,dchdate FROM acc_debtor WHERE account_code="1102050101.303" AND stamp = "N" GROUP BY vn');
    ...
    public function account_303_stam(Request $request)
    {
        $id = $request->ids;
        $iduser = Auth::user()->id;
        ...
        foreach ($data as $key => $value) {
            ...
            //  $check = Acc_debtor::where('vn', $value->vn)->where('account_code','1102050101.4011')->where('account_code','1102050101.4011')->count();
                $check = Acc_1102050101_303::where('vn', $value->vn)->count();
                if ($check > 0) {
                # code...
                } else {
                    Acc_1102050101_303::insert([
                            'vn'                => $value->vn,
                            'hn'                => $value->hn,
                            'an'                => $value->an,
                            'cid'               => $value->cid,
    ...
    public function account_303_search(Request $request)
    {
        $datenow = date('Y-m-d');
        $startdate = $request->startdate;
        $enddate = $request->enddate;
        ...
        if ($startdate =='') {
           $datashow = DB::select(' 
               SELECT * from acc_1102050101_303 
        ...
        return view('account_303.account_303_search', $data, [
            'startdate'     => $startdate,
            'enddate'       => $enddate,
            'datashow'      => $datashow,
            'startdate'     => $startdate,
    ...
    public function account_303_destroy_all(Request $request)
    {
        $id = $request->ids;
        $iduser = Auth::user()->id;
        $data = Acc_debtor::whereIn('acc_debtor_id',explode(",",$id))->get();
    ...
    public function account_303_detail(Request $request,$startdate,$enddate)
    {
        $datenow = date('Y-m-d');
        $startdate = $request->startdate;
        $enddate = $request->enddate;
        ...
        SELECT U1.vn,U1.hn,U1.cid,U1.ptname,U1.vstdate,U1.pttype,U1.debit_total
            from acc_1102050101_303 U1
        
            WHERE U1.vstdate BETWEEN "'.$startdate.'" and "'.$enddate.'"
            GROUP BY U1.vn
        ');
        // WHERE month(U1.vstdate) = "'.$months.'" and year(U1.vstdate) = "'.$year.'"
        return view('account_303.account_303_detail', $data, [ 
            'data'          =>     $data,
            'startdate'     =>     $startdate,
            'enddate'       =>     $enddate
        ]);
    ...
    public function account_303_detail_date(Request $request)
    { 
        $data['users'] = User::get();
        $startdate = $request->startdate;
        $enddate = $request->enddate;
        ...
        SELECT U1.vn,U1.hn,U1.cid,U1.ptname,U1.vstdate,U1.pttype,U1.debit_total
            ...
            GROUP BY U1.vn
        ');
        return view('account_303.account_303_detail_date', $data, [ 
            'data'           =>     $data,
            'startdate'      =>     $startdate,
            'enddate'        =>     $enddate
        ]);
