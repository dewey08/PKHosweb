...
class AccountController extends Controller
{
    ...
    public function account_info_vn(Request $request, $startdate, $enddate)
    {
        $datashow = DB::connection('mysql10')->select('
            select e.vn,e.hn,p.cid,e.pdx,e.vstdate,concat(p.pname,p.fname," ",p.lname) as fullname
                ...
                and e.uc_money > 0
                and (rr.sss_approval_code is null or rr.sss_approval_code ="")   
                and e.vn NOT IN(SELECT vn FROM ipt)                            
                group by e.vn 
    ...
    public function account_money_rep(Request $request,$id)
    {
        ...
        $data['users_hos'] = DB::table('users_hos')->get();
        $data_hos = DB::table('users_hos')->where('users_hos_id','=',$id)->first();
        // dd($main_type);
    ...
    public function account_money_copysave(Request $request)
    {
        ...
        //Check เดือนที่แล้ว มีข้อมูลไหม
        ...
        //Check ซ้ำ เดือนใหม่
        $yearcount = Account_main::where('account_main_year','=',$leave_yearid3)->where('store_id','=',$store_id)->count();
        $monthsid = Account_main::where('account_main_mounts','=',$MONTH_ID3)->where('store_id','=',$store_id)->count();
        $usergroup = Account_main::where('account_main_type','=',$account_maintype22)->where('store_id','=',$store_id)->count();
        ...
        if ($checkperson == 0) {
            ...
            // }else if($yearcount > 0 && $monthsid > 0 && $usergroup) {
            //     return response()->json([
            //         'status'     => '100'
            //     ]);
            } else {
    ...
    public function account_money_personcheckupdate(Request $request)
    {
        $id = $request->account_main_id;
        $acc = $request->acc;
        $salary = $request->salary;
    ...
    public function account_money_pay(Request $request,$id)
    {
        $startdate = $request->startdate;
        $enddate = $request->enddate;
        $main_type = $request->account_main_type;
        ...
        $data['users_hos'] = DB::table('users_hos')->get();
        $data_hos = DB::table('users_hos')->where('users_hos_id','=',$id)->first();
        return view('account.account_money_pay', $data, [
            'datashow'   =>  $datashow,
            'startdate'  =>  $startdate,
            'enddate'    =>  $enddate,
            'data_account_main' => $data_account_main,
    ...
    public function account_money_payupdate(Request $request)
    {
        ...
        $gbkhdata = DB::table('account_listpercen')->where('account_listpercen_id','=',1)->first();  //กบข./กสจ./สมทบ เงินเดือน
        $gbkh = $gbkhdata->account_listpercen_percent;
        ...
        $tax = $request->tax;
        // $fund = $request->fund;
        $fund =  $rev_salary * $gbkh / 100;     //กบข./กสจ./สมทบ เงินเดือน
        // $fundbackpay = $request->fundbackpay;
        $fundbackpay = $rev_backpay * $fundbackpaypercent / 100; //กบข ตกเบิก

        $addpercent = $request->add;

        if ($addpercent != '') {
            $summon = $rev_backpay + $rev_salary;
            $add =  $summon*$addpercent / 100; //ส่วนเพิ่ม
        } else {
            $add = 0;
        }
        ...
        Account_main::where('account_main_id', $id)
            ...
            // 'balance'          => $rev_sum - $tax+$fund+$fundbackpay+$add+$installment+$flat+$share+$loan+$food+$water+$electric+$coop+$F24+$F25+$F26+$F27+$F28+$F29+$other
        return response()->json([
            'status'     => '200'
        ]);
    }
    public function account_money_paycheckupdate(Request $request)
    {
    ...
    public function account_money_pay_onlyupdate(Request $request)
    {
        ...
        $tax = $request->tax;
        // $fund = $request->fund;
        $fund =  $rev_salary * $gbkh / 100;     //กบข./กสจ./สมทบ เงินเดือน * $a %
        // $fundbackpay = $request->fundbackpay;
        $fundbackpay = $rev_backpay * $fundbackpaypercent / 100; //กบข ตกเบิก * $a %
    ...
    public function account_money_setting (Request $request)
    {
        $data['account_percen'] = DB::table('account_percen')->get();
        $data['account_listpercen'] = DB::table('account_listpercen')->get();
        $data['account_creditor'] = DB::table('account_creditor')->get();
    ...
    public function account_money_creditoredit(Request $request,$id)
    {
        $creditor = Account_creditor::find($id);
    ...
    public function account_money_report(Request $request)
    {
        ...
        $data['users_hos'] = DB::table('users_hos')->get();
        // dd($main_type);
        return view('account.account_money_report', $data, [
            'datashow'   =>  $datashow,
            'startdate'  =>  $startdate,
            'enddate'    =>  $enddate,
            'data_account_main' => $data_account_main,
    ...
    {
        $data['users_hos'] = DB::table('users_hos')->get();
        return view('account.account_money_monthlydebthos', $data );
    }
    public function account_money_monthlydebt(Request $request,$id)
    {
        $startdate = $request->startdate;
        $enddate = $request->enddate;
        $main_type = $request->account_monthlydebt_type;
        // $hoscode = $request->store_id;
        $date = date('Y-m-d');
        $strM = date('m', strtotime($startdate));
        $strY = date('Y', strtotime($startdate)) + 543;
        ...
        if ($main_type != '' ) {
            $datashow = DB::connection('mysql')->select('
                select a.account_monthlydebt_id,a.account_monthlydebt_date,a.account_monthlydebt_time,a.account_monthlydebt_mounts
                    ,a.account_monthlydebt_year,a.account_monthlydebt_type,a.cid,a.person_name,a.shk_code,a.shk_price,a.tos_code,a.tos_price
        ...
        $data['account_creditor'] = DB::table('account_creditor')->get();
        // dd($main_type);
        return view('account.account_money_monthlydebt', $data, [
            'datashow'                 =>  $datashow,
            'startdate'                =>  $startdate,
            'enddate'                  =>  $enddate,
            'data_account_monthlydebt' => $data_account_monthlydebt,
            'strM'                     => $strM,
            'strY'                     => $strY,
            'main_type'                => $main_type,
            'id'                       => $id
    ...
    public function account_money_monthlydebtupdate(Request $request)
    {
        ...
        $tos_code = $request->tos_code;
        $os_price = $request->os_price;
        $os_code = $request->os_code;
        $ktb_price = $request->ktb_price;
        $ktb_code = $request->ktb_code;
        $cremation_price = $request->cremation_price;
        $cremation_code = $request->cremation_code;
        ...
        Account_monthlydebt::where('account_monthlydebt_id', $id)
            ->update([
                ...
                'ktb_price'        => $ktb_price,
                'ktb_code'         => $ktb_code,
                'cremation_price'             => $cremation_price,
                'cremation_code'            => $cremation_code,
    ...
    public function account_money_monthlydebt_personsave(Request $request)
    {
        $years_idold = $request->leave_year_id;
        $years_id = $years_idold;

        $months_id = $request->MONTH_ID;
        $user_group = $request->account_monthlydebt_type;
        $store_id = $request->store_id;

        $time = date("H:i:s");
        $date = date('Y-m-d');
        $yearcount = Account_monthlydebt::where('account_monthlydebt_year','=',$years_id)->count();
        ...
        if ($yearcount > 0 && $monthsid > 0 && $usergroup && $storeid > 0) {
            ...
            foreach ($users as $item) {
                ...
                $add->account_monthlydebt_time = $time;
                $add->account_monthlydebt_mounts = $months_id;
    ...
    public function account_money_monthlydebt_copypersonsave(Request $request)
    {
        $leave_yearid22 = $request->leave_year_id22;
        $MONTH_ID22 = $request->MONTH_ID22;
        $account_monthlydebt_type22 = $request->account_monthlydebt_type22;
        $store_id = $request->store_idCopy;
        ...
        //Check เดือนที่แล้ว มีข้อมูลไหม
        $checkperson = Account_monthlydebt::where('account_monthlydebt_year','=',$leave_yearid22)
        ->where('account_monthlydebt_mounts','=',$MONTH_ID22)
        ->where('account_monthlydebt_type','=',$account_monthlydebt_type22)
        ->where('store_id','=',$store_id)
        ->count();

        //Check ซ้ำ เดือนใหม่
        $yearcount = Account_monthlydebt::where('account_monthlydebt_year','=',$leave_yearid3)->where('store_id','=',$store_id)->count();
        $monthsid = Account_monthlydebt::where('account_monthlydebt_mounts','=',$MONTH_ID3)->where('store_id','=',$store_id)->count();
        $usergroup = Account_monthlydebt::where('account_monthlydebt_type','=',$account_monthlydebt_type22)->where('store_id','=',$store_id)->count();
        ...
        if ($checkperson == 0) {
            ...
            //     ]);
                ...
                foreach ($users_old as $item) {
                    ...
                    $add->account_monthlydebt_date = $date;
                    $add->cid = $item->cid;
                    $add->account_monthlydebt_year = $leave_yearid3;
                    $add->account_monthlydebt_time = $time;
                    $add->account_monthlydebt_mounts = $MONTH_ID3;
                    $add->account_monthlydebt_type = $account_monthlydebt_type22;
                    // $add->person_name = $item->fname.' '.$item->lname;
                    $add->cid = $item->cid;
                    $add->person_name = $item->person_name;

                    $add->shk_code = $item->shk_code;
                    $add->shk_price = $item->shk_price;
                    $add->tos_code = $item->tos_code;
                    $add->tos_price = $item->tos_price;
                    $add->os_code = $item->os_code;
                    $add->os_price = $item->os_price;
                    $add->ktb_code = $item->ktb_code;
                    $add->ktb_price = $item->ktb_price;
                    $add->cremation_code = $item->cremation_code;
                    $add->cremation_price = $item->cremation_price;
                    $add->other_code = $item->other_code;
                    $add->other_price = $item->other_price;
                    $add->scb_code = $item->scb_code;
