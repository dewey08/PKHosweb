...
class Account502Controller extends Controller
 { 
    ...
    public function account_502_pull(Request $request)
    {
        ...
        if ($startdate == '') {
            ...
            $acc_debtor = DB::select('
                ...
                WHERE a.account_code="1102050101.502"
                AND a.stamp = "N"
                group by a.an
        ...
        return view('account_502.account_502_pull',[
            'startdate'     =>     $startdate,
            'enddate'       =>     $enddate,
    ...
    public function account_502_pulldata(Request $request)
    {
        $datenow = date('Y-m-d');
        ...
        $acc_debtor = DB::connection('mysql2')->select('             
                ...
                ,"21" as acc_code
                ,"1102050101.502" as account_code
                ,"ลูกหนี้ค่ารักษา-คนต่างด้าวและแรงงานต่างด้าว IP" as account_name
        ...
        foreach ($acc_debtor as $key => $value) {
            if ($value->debit >0) {
                $check = Acc_debtor::where('an', $value->an)->where('account_code','1102050101.502')->whereBetween('dchdate', [$startdate, $enddate])->count();
                if ($check == 0) {
                    Acc_debtor::insert([
                        'hn'                 => $value->hn,
                        'an'                 => $value->an,
    ...
    public function account_502_stam(Request $request)
    {
        $id = $request->ids;
        $iduser = Auth::user()->id;
        ...
        foreach ($data as $key => $value) {
             ...
             $check = Acc_1102050101_502::where('an', $value->an)->count(); 
                if ($check > 0) {
                # code...
                } else {
    ...
    public function account_502_detail(Request $request,$months,$year)
    {
        $datenow = date('Y-m-d');  
        $data['users'] = User::get();  
        $data = DB::select('
            SELECT U1.an,U1.vn,U1.hn,U1.cid,U1.ptname,U1.vstdate,U1.dchdate,U1.pttype,U1.debit_total ,U1.adjrw,U1.total_adjrw_income,U1.income,U1.rcpt_money
            from acc_1102050101_502 U1            
            WHERE month(U1.dchdate) = "'.$months.'" AND year(U1.dchdate) = "'.$year.'"
            GROUP BY U1.an
        ');
      
        return view('account_502.account_502_detail', $data, [ 
            'data'       =>     $data,
            'months'     =>     $months,
            'year'       =>     $year
        ]);
    ...
    //     ]);
    // }
    public function account_502_detail_date(Request $request,$startdate,$enddate)
    {
        $datenow = date('Y-m-d');  
        $data['users'] = User::get();  
        $data = DB::select('
            SELECT U1.an,U1.vn,U1.hn,U1.cid,U1.ptname,U1.vstdate,U1.dchdate,U1.pttype,U1.debit_total ,U1.income,U1.rcpt_money
            from acc_1102050101_502 U1            
            WHERE U1.dchdate BETWEEN "'.$startdate.'" AND "'.$enddate.'"
            GROUP BY U1.an
        ');
      
        return view('account_502.account_502_detail_date', $data, [ 
            'data'          =>  $data,
            'startdate'     =>  $startdate,
            'enddate'       =>  $enddate
        ]);
